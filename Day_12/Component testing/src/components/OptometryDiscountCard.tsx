...
export const OptometryDiscountCard = ({ frameModel, initialPriceUSD }: Props) => {
  ...
  return (
    <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl text-white font-mono max-w-sm">
      <h3 className="text-cyan-400 font-bold">👓 إطار: {frameModel}</h3>
      
      <p className="mt-2 text-sm text-slate-300">
        السعر الحالي: <span data-testid="price-display" className="font-extrabold text-amber-400">${price}</span>
      </p>
      ...
      >
        {isDiscountApplied ? "تم تطبيق الخصم ✅" : "تطبيق خصم 20% 🏷️"}
      </button>
    </div>
  );
};