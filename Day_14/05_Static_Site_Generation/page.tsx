...
/**
 * ==========================================
 * STATIC SITE GENERATION (SSG) MECHANICS
 * ==========================================
 * Static Site Generation builds the HTML for a page at build time (when you run `npm run build`),
 * rather than on every request. This is the default behavior in Next.js for maximum performance.
 * 
 * For dynamic routes (like `[slug]`), Next.js doesn't inherently know which URLs to pre-build.
 * We use `generateStaticParams` to return a list of all possible parameters. 
 * Next.js will then iterate through this list and statically generate an HTML file for each one.
 * 
 * Why use SSG?
 * - Lightning fast load times (served from a CDN).
 * - Great for SEO.
 * - Reduced server load.
 */
...
interface PageProps {
  // `params` will receive the values generated by `generateStaticParams` during build time.
  params: {
    slug: string;
  };
}
