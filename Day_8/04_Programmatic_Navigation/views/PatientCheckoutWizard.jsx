...
const PatientCheckoutWizard = ({ patientId = "884", amount = "150.00" }) => {
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      console.log("ðŸ’³ Payment Approved! Triggering automated UX redirect...");
      ...
      alert("âš ï¸ Payment failed. Please check clinic connection.");
    } finally {
  ...
  const handleCancel = () => {
    navigate(-1); // Works exactly like the browser's back button ðŸ”™
  };
  ...
  return (
    <div className="p-8 bg-slate-900 rounded-2xl border border-slate-800 font-mono text-white max-w-md mx-auto shadow-2xl">
      <div className="flex justify-between items-center border-b border-slate-800 pb-4 mb-6">
        <h3 className="text-lg font-bold text-emerald-400">ðŸ’³ Checkout & Billing</h3>
      ...
      <div className="flex gap-3">
        ...
        >
          {isProcessing ? "â³ Charging..." : "âœ… Confirm Payment"}
        </button>
      </div>
    </div>
  );
