...
// 1. استدعاء كاميرا القراءة (useSelector) وهاتف الأوامر (useDispatch):
import { useSelector, useDispatch } from 'react-redux';
import { sellFrame, addFrame } from '../store/inventorySlice';

const FramesInventoryView = () => {
  // 📸 قراءة البيانات من الخزنة (نطلب فقط قسم المخزون!):
  const { frames, totalValue } = useSelector((state) => state.inventory);
  
  // 📞 إمساك هاتف الأوامر لإرساله للخزنة:
  const dispatch = useDispatch();

  const handleSell = (id) => {
    // إرسال أمر "بيع إطار" مع إرفاق الـ ID كـ Payload:
